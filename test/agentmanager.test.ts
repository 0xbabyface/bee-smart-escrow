...
describe("AgentManager", async function () {
  ...
  describe("normal", function () {
    ...
    it("normal operations", async function () {
      ...
      await manager.connect(operator).addAgent(topAgent.address, agent2.address, StarLevel.Star2, false, "sub agent2");
      ...
      expect(agent[6]).to.equal('sub agent2');

      await manager.connect(operator).addAgent(agent2.address, agent3.address, 2, true, 'sub agent 3');
      await manager.connect(operator).addAgent(agent3.address, agent4.address, 2, true, 'sub agent 4')
