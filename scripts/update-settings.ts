...
async function main() {
  ...
  // edmond settings
  tx =  await smart.setRole(await smart.AdminRole(), '0x7413521899ded2719a5BFaEAe6a8375A0Be622dD', true, {gasPrice: 30000000000}); // edmond
  await tx.wait();
  tx = await smart.setRole(await smart.CommunityRole(), '0xacB93B394cFc772C2273f54389c3081aa92Cf0C8', true, {gasPrice: 30000000000}); // edmond
  await tx.wait();
  // tx = await agentManager.addTopAgent('0xBBB553176222Cd4eBE601ED224CE549A2f738776', 3, true, 'edmond');
  // await tx.wait();


  // enpeng settings
  tx = await smart.setRole(await smart.CommunityRole(), '0x6B26D93522511fa0c64107235402c40B18Cd42b3', true, {gasPrice: 30000000000});
  await tx.wait();
  tx = await smart.setRole(await smart.CommunityRole(), '0x55a0e984bC0C2650a16E2300a372b1D94CbD3E63', true, {gasPrice: 30000000000});
  await tx.wait();
  tx = await smart.setRole(await smart.AdminRole(), '0x55a0e984bC0C2650a16E2300a372b1D94CbD3E63', true, {gasPrice: 30000000000});
  await tx.wait();
  // tx = await agentManager.addTopAgent('0xF2b3CCBc45bdA7d0aa5a25172C0a270ad55D2473', 3, 'enpeng', '0xF2b3CCBc45bdA7d0aa5a25172C0a270ad55D2473');
  // await tx.wait();
  ...
  // await tx.wait();
  // tx = await agentManager.addTopAgent('0x81bD01D0A9E8e8E40FAf22B779Bb21BaFbf8f7AC', 3, true, 'shifu');
  // await tx.wait();
  ...
  console.log(`agent: ${await agentManager.getAgentByWallet('0xb20191CbeD6c9d9122b787Ee2bbbD758FD5287CE')}`)
